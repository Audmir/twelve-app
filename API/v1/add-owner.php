...
$proprietaire_id = htmlspecialchars($_GET['prop_id']);
...
try {
    if ($method === "GET") {
        if (
            !empty($proprietaire_id) || !empty($nom_complet) || !empty($date_de_naissance) ||
            !empty($Lieu_de_naissance) || !empty($adresse) || !empty($etat_civil) || !empty($phone) ||
            !empty($email) || !empty($photo)
        ) {

            $query = $conn->prepare("INSERT INTO proprietaire(`proprietaire_id`,`nom_complet`,`data_de_naissance`,
            `Lieu_de_naissance`,`Adresse`,`Etat_civile`,`phone`,`email`,`photo`)
            VALUES('{$proprietaire_id}','{$nom_complet}', '{$date_de_naissance}', '{$Lieu_de_naissance}',
